...
class BaseRepository(Generic[SchemaType, CreateInputType, UpdateInputType]):
    ...
    def _create(self, db_session: Session, inp: CreateInputType) -> SchemaType:
    ...
    def _update(
        ...
        db_obj = self._find_one_by_id(db_session, id_)
        if db_obj is None:
            return None
    ...
    def _delete(self, db_session: Session, id_: ID) -> None:
        stmt = (
    ...
    def _find_all(self, db_session: Session) -> List[SchemaType]:
        return (
    ...
    def _find_one_by_id(self, db_session: Session, id_: ID) -> Optional[SchemaType]:
