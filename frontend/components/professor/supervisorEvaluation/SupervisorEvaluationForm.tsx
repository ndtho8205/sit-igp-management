...
class ScoreInput extends React.Component<any, any> {
  ...
  constructor(props: { scoreData: any , criteria: any , isThesis: boolean }) {
    ...
    this.state = {
      ...
      score_p1: props.scoreData[(props.isThesis)? 'thesis_program':'lab_seminar'].score_p1 as number,
      score_d2: props.scoreData[(props.isThesis)? 'thesis_program':'lab_seminar'].score_d2 as number,
      score_p2: props.scoreData[(props.isThesis)? 'thesis_program':'lab_seminar'].score_p2 as number,
      score_course: props.scoreData[(props.isThesis)? 'thesis_program':'lab_seminar'].score_course as number,
    };
    this.hasLabRotation = (props.scoreData.school_year == SchoolYear.Freshmen_1) ? false : 
                          (props.scoreData.school_year == SchoolYear.Freshmen_2) ? false : true;
  }
  ...
  calculateCourseScore = () => {
    setTimeout( function (this : ScoreInput){
      ...
      if (this.props.isThesis){
        score_course = Math.round(this.calculateThesisCourseScore()*100)/100;
      }
      else{
        score_course = Math.round(this.calculateLabSeminarCourseScore()*100)/100;
      }
      this.setState({
  ...
  calculateThesisCourseScore = () : number => {
    ...
    const originalCourseScore = score_d1*0.35 + score_d2*0.35 + 
                                score_p1*0.1 + score_p2*0.1 + 
                                this.props.scoreData.score_presentation*0.1;
    if(this.hasLabRotation) {
      return (originalCourseScore*(5/6)) +
  ...
  calculateLabSeminarCourseScore = () => {
    const { score_d1, score_d2, score_p1, score_p2 } = this.state;
    const originalCourseScore = score_d1*0.35 + score_d2*0.15 + 
                                score_p1*0.3 + score_p2*0.1 + 
                                this.props.scoreData.score_presentation*0.1;
    if(this.hasLabRotation) {
      return (originalCourseScore*0.5) +
      ...
      return (
        <p className={styles.supervisorEvaluationFormCell}>
          <Form.Item
            ...
            name={cellItem.value}
            initialValue={this.props.scoreData[(this.props.isThesis)? 'thesis_program':'lab_seminar'][cellItem.value]}
            style={{margin: 0}}
          >
            <InputNumber 
              min={0} 
              ...
              if (noOfCol < 5){
                ...
                rows.push(
                  <Col span={24} style={{border:"1px solid black"}}>
                    {this.generateCell({type: Cell.FormItem, value: "score_course", disabled: true, label: "Course Score"})}
                  </Col>
                );
                return rows;
              }
                ...
                return(
                  <>
                    <Col span={20}>
                      <Row>
                        {this.generateRow(Header,noOfCol - 1)}
                      </Row>
                    </Col>
                    ...
                    <Col span={4} style={{border:"1px solid black"}}>
                      {this.generateCell({type: Cell.Text, value: this.props.scoreData.score_lab_rotation})}
                    </Col>
